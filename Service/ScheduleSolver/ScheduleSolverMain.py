...
from Service.ScheduleSolver.RecursiveAlgorithm.ParallelScheduleSolverRecursive import ParallelScheduleSolverRecursive
...
from Service.Utils.ImportantStepsFinder import ImportantStepsFinder
from Service.Utils.ResolvedStepsCropper import ResolvedStepsCropper
...
class ScheduleSolverMain:
    def __init__(self, factory: Factory, moved_steps: Dict[UUID, MovedStep]):
        ...
        self.schedule_solver_optimal = ScheduleSolverOptimal(self.factory_info_provider)
        self.schedule_solver_recursive = ParallelScheduleSolverRecursive(self.factory_info_provider) #ScheduleSolverRecursive(self.factory_info_provider)
        self.factory_cropper = FactoryCropper()
        self.factory_creator = FactoryCreator()
        self.resolved_steps_cropper = ResolvedStepsCropper()
        self.resolved_steps_evaluator = ResolvedStepsEvaluator()
        self.important_steps_finder = ImportantStepsFinder()
        logging.info("Finished ScheduleSolverMain")

    def solve_optimal(self, max_search_time=40) -> ([ResolvedStep], str):
        self.schedule_solver_optimal.maxSearchTime = max_search_time
        resolved_steps, status = self.schedule_solver_optimal.solve_optimal()
        changed_steps = self.resolved_steps_cropper.leave_only_changed_steps(resolved_steps, self.factory_info_provider)
        return changed_steps, status
    ...
    def solve_for_moved_steps(self, moved_steps: [MovedStep], movement_type: MovementType, max_search_time=3) -> (
        ...
        resolved_steps, status = self.schedule_solver_recursive.solve_for_moved_steps(movement_type,
        ...
        resolved_steps_evaluation = self.resolved_steps_evaluator.evaluate(resolved_steps_without_magnetic, self.factory_info_provider)
        logging.info(f"resolved_steps_evaluation {resolved_steps_evaluation}")

        resolved_steps, status = self.schedule_solver_optimal.adjust_solving_for_magnetic(
            resolved_steps_without_magnetic, magnetic_constraints, max_search_time)

        if status == "OPTIMAL" or status == "FEASIBLE":
            changed_steps = self.resolved_steps_cropper.leave_only_changed_steps(resolved_steps,
                                                                                 self.factory_info_provider)
            return changed_steps, status
    ...
    def resolve_conflicts(self, max_search_time=3) -> ([ResolvedStep], str):
        ...
        if not self.fast_determine_possibility(max_search_time):
            max_search_time = 0.5

        # magnetic_constraints изменяется
        resolved_steps, status = self.schedule_solver_recursive.resolve_conflicts(magnetic_constraints,
                                                                                  max_search_time)
    ...
    def solve_for_sorted_steps(self, sorted_steps: [UUID], max_search_time=3) -> ([ResolvedStep], str):
        ...
        # magnetic_constraints изменяется
        resolved_steps, status = self.schedule_solver_recursive.solve_for_sorted_steps(sorted_steps,
                                                                                       magnetic_constraints,
                                                                                       max_search_time)
    ...
    def solve_optimal_for_demands(self, max_search_time=3) -> ([ResolvedStep], str):
        ...
        # magnetic_constraints изменяется
        resolved_steps, status = self.schedule_solver_recursive.solve_optimal_for_demands(magnetic_constraints,
                                                                                          max_search_time)
        ...
        if status != "OPTIMAL" and status != "FEASIBLE":
            ...
            changed_optimal_steps = self.resolved_steps_cropper.leave_only_changed_steps(optimal_resolved_steps,
                                                                                 self.factory_info_provider)
            return changed_optimal_steps, optimal_status
    ...
    def fast_determine_possibility(self, max_search_time=3):
        logging.info("Starting fast_determine_possibility")

        cropped_factory = self.important_steps_finder.crop_factory_with_important_steps(self.schedule_solver_recursive)
        logging.info(f"cropped_factory len: {len(cropped_factory.steps)}")
        logging.info(f"cropped_factory steps: {cropped_factory.steps.keys()}")
        logging.info(f"cropped_factory steps: {cropped_factory.steps.values()}")

        cropped_optimal_solver = ScheduleSolverOptimal(FactoryInfoProvider(cropped_factory, {}))
        cropped_resolved_steps, status = cropped_optimal_solver.solve_first()
